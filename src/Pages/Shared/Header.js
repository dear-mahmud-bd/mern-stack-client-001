            ...
            <div className='hidden md:flex w-full h-full items-center justify-between'>
                ...
                <div className='flex items-center gap-8'>
                    ...
                    <div className='relative'>
                        <motion.img whileTap={{ scale: 0.6 }} src={UserPic} className='w-11 h-11 min-w-[40px] min-h-[40px] rounded-full drop-shadow-2xl cursor-pointer' alt="User Profile" onClick={dropdownMenu} />
                                    ...
                                    <motion.div initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.6 }} className='w-40 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute top-11 right-0'>
                                        {
                                            user ?
                                                <>
                                                    <Link to='/myitem'><p className='px-4 py-2 rounded-lg flex items-center gap-2 cursor-pointer hover:bg-slate-200 transition-all duration-200 ease-in-out text-textColor text-base' onClick={() => setIsMenu(false)}><CgUserList /> My Item</p></Link>
                    ...
                    <div className=''>
                        {
                            isMenu && (
                                <motion.div initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.6 }} className='w-40 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute top-11 right-0'>
                                    ...
                                    <Link to='allitem'><p className='px-4 py-2 rounded-lg flex items-center gap-2 cursor-pointer hover:bg-slate-200 transition-all duration-200 ease-in-out text-textColor text-base' onClick={() => setIsMenu(false)}><MdList /> All Item</p></Link>
                                    {
                                        user ?
                                            <>
                                                <Link to='/myitem'><p className='px-4 py-2 rounded-lg flex items-center gap-2 cursor-pointer hover:bg-slate-200 transition-all duration-200 ease-in-out text-textColor text-base' onClick={() => setIsMenu(false)}><CgUserList /> My Item</p></Link>
